...
for (snr in c(0.5,1,2,3,4,5,6,7,8)) {
  file_path <- paste0("OriginalResults/OtherMethods/LowC1/Result-n-100-p-20-SNR-", snr, "-rho-0.8-case-1-nsim-50.RData")
  load(file_path)
...
MCC_LD
ggsave("MCC-LD-Case-1.png", plot = MCC_LD, dpi = 1200)
...
PE_LD
ggsave("PE-LD-Case-1.png", plot = PE_LD, dpi = 1200)
```
...
ACC_LD
ggsave("ACC-LD-Case-1.png", plot = PE_LD, dpi = 1200)
```
...
F1_LD
ggsave("F1-LD-Case-1.png", plot = F1_LD, dpi = 1200)
```
...
SE_LD
ggsave("SE-LD-Case-1.png", plot = SE_LD, dpi = 1200)
```
...
SP_LD
ggsave("SP-LD-Case-1.png", plot = SP_LD, dpi = 1200)
```