...
N <- 50
for (snr in c(0.5,1,2,3,4,5,6,7,8)) {
  COMBSSV1_path = paste0("ThesisResults/COMBSSV1-Optimal/COMBSSV1-Scaled-Case-2-n-100-p-20-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50-nlam-25-eta-0.001.csv")
...
N <- 50
for (snr in c(0.5,1,2,3,4,5,6,7,8)) {
  COMBSSV2_path = paste0("ThesisResults/COMBSSV2-Optimal/COMBSSV2-Improved-Case-2-n-100-p-20-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50-nlam-25-eta-0.001.csv")
...
MCC_LD
ggsave("CH6-MCC-LD-Case-2.png", plot = MCC_LD, dpi = 1200)
...
PE_LD
ggsave("CH6-PE_LD-Case-2.png", plot = PE_LD, dpi = 1200)
```
...
F1_LD
ggsave("CH6-F1_LD-Case-2.png", plot = F1_LD, dpi = 1200)
```
...
ACC_LD
ggsave("CH6-ACC_LD-Case-2.png", plot = ACC_LD, dpi = 1200)
```
...
SE_LD
ggsave("CH6-SE_LD-Case-2.png", plot = SE_LD, dpi = 1200)
```
...
ggplot(SP_Data, aes(x = SNR, y = Mean, color = Method)) +
  geom_line(size = 0.5) +  # Thinner lines
  geom_point(size = 0.5) +  # Smaller points
  scale_x_continuous(breaks = unique(SP_Data$SNR)) +
  geom_pointrange(aes(ymin = Lower, ymax = Upper), size = 0.15, show.legend = FALSE) +  # Adjust error bar thickness
...
SP_LD
ggsave("CH6-SP_LD-Case-2.png", plot = SP_LD, dpi = 1200)
```
...
TIME_LD
ggsave("CH6-TIME_LD-Case-2.png", plot = TIME_LD, dpi = 1200)
```