...
N <- 50
for (snr in 2:8) {
  combss_path = paste0("OriginalResults/COMBSS/COMBSS-case-2-n-100-p-1000-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50.csv")
...
N <- 50
for (snr in 2:8) {
  combss_path = paste0("ThesisResults/COMBSS-Original/COMBSSV0-case-2-n-100-p-1000-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50-nlam-25-eta-0.001.csv")
  combss_data <- read.csv(combss_path)
...
ggplot(MCC_Data, aes(x = SNR, y = Mean, color = Method)) +
  ...
  scale_color_manual(values = c(
    ...
    "COMBSS with Feature Scaling" = "gold"
  )) +
  theme(aspect.ratio = 1)

MCC_HD
ggsave("CH4-MCC-HD-Case-2.png", plot = MCC_HD, dpi = 1200)
...
PE_HD
ggsave("CH4-PE_HD-Case-2.png", plot = PE_HD, dpi = 1200)
```
...
F1_HD
ggsave("CH4-F1_HD-Case-2.png", plot = F1_HD, dpi = 1200)
```
...
SE_HD
ggsave("CH4-SE_HD-Case-2.png", plot = SE_HD, dpi = 1200)
```
...
SP_HD
ggsave("CH4-SP_HD-Case-2.png", plot = SP_HD, dpi = 1200)
```
...
ggplot(TIME_Data, aes(x = SNR, y = Mean, color = Method)) +
  ...
  scale_color_manual(values = c(
    "Original COMBSS" = "salmon",
    "COMBSS, Refined Gradient" = "gold"
  )) +
  theme(aspect.ratio = 1)

TIME_HD
ggsave("CH4-TIME_HD-Case-2.png", plot = TIME_HD, dpi = 1200)
```