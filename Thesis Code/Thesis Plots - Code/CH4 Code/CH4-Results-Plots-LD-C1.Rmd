...
for (snr in c(0.5,1,2,3,4,5,6,7,8)) {
  combss_path = paste0("OriginalResults/COMBSS/COMBSS-case-1-n-100-p-20-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50.csv")
...
N <- 50
for (snr in c(0.5,1,2,3,4,5,6,7,8)) {
  combss_path = paste0("ThesisResults/COMBSS-Original/COMBSSV0-case-1-n-100-p-20-q-20-corr-0.8-ninit-1-snr-", snr, "-ndatasets-50-nlam-25-eta-0.001.csv")
  combss_data <- read.csv(combss_path)
...
MCC_LD
ggsave("CH4-MCC-LD-Case-1.png", plot = MCC_LD, dpi = 1200)
...
PE_LD
ggsave("CH4-PE_LD-Case-1.png", plot = PE_LD, dpi = 1200)
```
...
F1_LD
ggsave("CH4-F1_LD-Case-1.png", plot = F1_LD, dpi = 1200)
```
...
ACC_LD
ggsave("CH4-ACC_LD-Case-1.png", plot = ACC_LD, dpi = 1200)
```
...
SE_LD
ggsave("CH4-SE_LD-Case-1.png", plot = SE_LD, dpi = 1200)
```
...
SP_LD
ggsave("CH4-SP_LD-Case-1.png", plot = SP_LD, dpi = 1200)
```
...
TIME_LD
ggsave("CH4-TIME_LD-Case-1.png", plot = TIME_LD, dpi = 1200)
```